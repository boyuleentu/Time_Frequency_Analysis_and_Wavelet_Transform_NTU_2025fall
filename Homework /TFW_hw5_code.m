image = double(imread('Peppers.bmp'));
[x1L, x1H1, x1H2, x1H3] = wavedbc10(image);
figure;
colormap(gray);
subplot(2,2,1);
imshow(x1L/255);
...
subplot(2,2,2);
imshow(x1H1); 
title('LH (Horizontal Edges)');
subplot(2,2,3);
imshow(x1H2); 
title('HL (Vertical Edges)'); 
subplot(2,2,4);
imshow(x1H3); 
title('HH (Diagonal Edges)');
x = iwavedbc10(x1L, x1H1, x1H2, x1H3);
figure;
imshow(x/255);

function [x1L, x1H1, x1H2, x1H3] = wavedbc10(x)
...
function x = iwavedbc10(x1L, x1H1, x1H2, x1H3)
